...
function PokazProdukty($link) {
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        // Przygotowanie komórki ze zdjęciem
        $zdjecie_html = '';
        if (!empty($row['zdjecie_blob'])) {
            $zdjecie_html = '<img src="data:image/jpeg;base64,'.base64_encode($row['zdjecie_blob']).'" width="50"/>';
        } else {
            $zdjecie_html = '<img src="'.$row['zdjecie_link'].'" width="50"/>';
        }
...
function ZarzadzajProdukty($link, $mode, $id = 0) {
    ...
    if (isset($_POST['product_submit'])) {
        ...
        $zdjecie_link = mysqli_real_escape_string($link, $_POST['zdjecie_link']);
        $data_wyg = mysqli_real_escape_string($link, $_POST['data_wygasniecia']);
        
		$imgUpdatePart = ""; 
		$imgInsertValue = "NULL";

		if (isset($_FILES['zdjecie_plik']) && $_FILES['zdjecie_plik']['error'] == 0 && !empty($_FILES['zdjecie_plik']['tmp_name'])) {
			$imgData = file_get_contents($_FILES['zdjecie_plik']['tmp_name']);
			$imgDataEncoded = mysqli_real_escape_string($link, $imgData);
			$imgUpdatePart = ", zdjecie_blob='$imgDataEncoded'";
			$imgInsertValue = "'$imgDataEncoded'";
		}

        if ($mode === 'add') {
            $query = "INSERT INTO produkty (tytul, opis, cena_netto, podatek_vat, ilosc_sztuk, status_dostepnosci, kategoria_id, gabaryt_produktu, zdjecie_link, data_wygasniecia, zdjecie_blob) 
                      VALUES ('$tytul', '$opis', $cena_netto, $podatek_vat, $ilosc, $status, $kategoria, '$gabaryt', '$zdjecie_link', '$data_wyg', $imgInsertValue)";
        } else {
            $query = "UPDATE produkty SET tytul='$tytul', opis='$opis', cena_netto=$cena_netto, podatek_vat=$podatek_vat, ilosc_sztuk=$ilosc, 
                      status_dostepnosci=$status, kategoria_id=$kategoria, gabaryt_produktu='$gabaryt', zdjecie_link='$zdjecie_link', data_wygasniecia='$data_wyg' $imgUpdatePart WHERE id = $id";
        }

        if (mysqli_query($link, $query)) {
            return '<p style="color: green;">Zapisano pomyślnie!</p>' . PokazProdukty($link);
        } else {
            $output .= '<p style="color: red;">Błąd SQL: ' . mysqli_error($link) . '</p>';
        }
    }
    ...
    <form method="post" enctype="multipart/form-data">
        <table class="cms_table">
			...
			<tr><td>Wgraj zdjęcie (z dysku):</td><td><input type="file" name="zdjecie_plik"></td></tr>
