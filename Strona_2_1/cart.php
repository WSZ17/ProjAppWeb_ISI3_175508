...
function PokazProduktySklep($link) {
    ...
    // Pobieramy produkty zgodnie z Twoją strukturą
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        // Obliczamy cenę brutto (netto + VAT)
        $cena_brutto = $row['cena_netto'] + ($row['cena_netto'] * ($row['podatek_vat'] / 100));
        
        // --- NOWA LOGIKA WYŚWIETLANIA ZDJĘCIA ---
        $zdjecie_html = '';
        if (!empty($row['zdjecie_blob'])) {
            // Jeśli jest zdjęcie w bazie, kodujemy je do Base64
            $zdjecie_html = '<img src="data:image/jpeg;base64,'.base64_encode($row['zdjecie_blob']).'" width="100" style="border-radius:4px;">';
        } else {
            // Jeśli nie ma bloba, używamy linku (nawet jeśli link jest pusty, wyświetli się ramka/alt)
            $zdjecie_html = '<img src="'.$row['zdjecie_link'].'" width="100" style="border-radius:4px;">';
        }
        // ----------------------------------------
 ...
 */
/**
 * Wyświetla koszyk z przyciskami +/- oraz synchronizacją z bazą
 */
function PokazKoszyk($link) {
    ...
    if (!isset($_SESSION['count']) || $_SESSION['count'] == 0) return "<p>Koszyk jest pusty.</p>";

    $jest_cos = false;
    for ($i = 0; $i < $_SESSION['count']; $i++) {
        if (isset($_SESSION[$i.'_1'])) {
            $jest_cos = true;
            $id_p = $_SESSION[$i.'_1']; // ID produktu z bazy danych
            $ile = $_SESSION[$i.'_2'];  // Ilość w koszyku
            ...
            if ($row = mysqli_fetch_assoc($res)) {
                ...
                $output .= '<tr>
                    ...
                    <td>
                        <a href="index.php?id=koszyk&action=decrease&item_id='.$i.'&id_prod='.$id_p.'" style="text-decoration:none; padding: 5px; font-weight:bold;">[-]</a>
                        '.$ile.'
                        <a href="index.php?id=koszyk&action=increase&item_id='.$i.'&id_prod='.$id_p.'" style="text-decoration:none; padding: 5px; font-weight:bold;">[+]</a>
                    </td>
                    <td>'.number_format($wartosc, 2).' zł</td>
                    <td><a href="index.php?id=koszyk&action=remove&item_id='.$i.'">Usuń wszystko</a></td>
                </tr>';
            }
        }
    }
